...
random.seed(0)
data_pan = pd.read_csv('../data/final_dataset.csv',sep=',',names=['S0','S1','S2','S3','exp','syn','Tg','Density','Solu','Tc'],skiprows=1)
#  print(data_pan)
...
def generate_dict(location,lang):
    ...
    OH_ALL.to_csv(f'../data/{lang}_dictionary')
...
#  for d in data_ind:
#    test,train = d
#    data_pan.iloc[train].to_csv(f'../data/Dc_train_{n}.csv')
#    data_pan.iloc[test].to_csv(f'../data/Dc_test_{n}.csv')
#    n +=1
for lang in ['S2','S0']:
    print(lang)
    OH_ALL,N_CHAR,MAX_LEN=generate_dict('../data/final_dataset.csv',lang)
    print(N_CHAR,MAX_LEN)
